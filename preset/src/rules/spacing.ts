...
// Helper to find a size by label in the utopia-core space scale
export const findSpaceSize = (label: string, options: RequiredOptions) => {
  if (!options.spaceScale)
    return

  // Check regular sizes
  const size = options.spaceScale.sizes.find(s => s.label === label)
  if (size)
    return size.clamp

  // Check one-up pairs
  const oneUp = options.spaceScale.oneUpPairs.find(s => s.label === label)
  if (oneUp)
    return oneUp.clamp

  // Check custom pairs
  const custom = options.spaceScale.customPairs.find(s => s.label === label)
  if (custom)
    return custom.clamp
}

export const fluidSpaceHandler = (match: string) => (options: RequiredOptions) => {
  return findSpaceSize(match, options)
}
